...
kmeans = joblib.load('x3_1_3000_kmeans.model')
pca_matrix = io.loadmat('x3_1_3000_patch_PCA_matrix.mat')['p']
mean = io.loadmat('x3_1_3000_patch_mean_matrix.mat')['m']
...
# Training procedure for each image (use numba.jit to speed up)
#@nb.jit(nopython=True, parallel=True)
def TrainProcessSMC (im_LR, im_HR, im_GX, im_GY,patchSize, w, Qangle, Qstrength,Qcoherence, stre, cohe, R, Q, V, mark):
    H, W = im_HR.shape
    for i1 in range(H-2*floor(patchSize/2)):
        for j1 in range(W-2*floor(patchSize/2)):
            idx1 = (slice(i1,(i1+2*floor(patchSize/2)+1)),slice(j1,(j1+2*floor(patchSize/2)+1)))
            ...
            theta,lamda,u=HashTable(patchX, patchY, w, Qangle, Qstrength,Qcoherence, stre, cohe)
            patch1 = patch.ravel()
            patchSubMean = patch1 - mean.reshape(1, -1)
            patch_reduced = np.dot(pca_matrix, patchSubMean.reshape(-1, 1))
            t = kmeans.predict(patch_reduced)
            patchL = patch1.reshape((1,patch1.size))
            #t = (i1 % R) * R +(j1 % R)
            j = theta * Qstrength * Qcoherence + lamda * Qcoherence + u
            tx = np.int(t)
